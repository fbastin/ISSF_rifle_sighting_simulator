...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Mechanical sight adjustment}---how ``clicks'' on the
        rear diopter translate into impact shifts on the target
        (\cref{sec:clicks}).
...
\key{Z}\,/\,\key{X}  & Rifle cant $\pm 0.57^{\circ}$
                      & unlimited
                      & \cref{sec:cant} \\
Arrow keys            & Rear-sight clicks $\pm 1$
                      & unlimited
                      & \cref{sec:clicks} \\
\key{G}\,/\,\key{H}  & Click value $\pm 0.01$\,mm
                      & 0.01--0.10\,mm
                      & \cref{sec:clicks} \\
...
group center onto the target center with the \emph{rear} sight---the
...
changing the angle between the bore and the line of
sight~\cite{nra}.  Match diopters move a few hundredths of a
millimetre per click: the simulator default is $c = 0.04$\,mm, and the
value is adjustable with \key{G}/\key{H} over $0.01$--$0.10$\,mm.
...
Understanding the \emph{lever-arm geometry} helps the shooter predict
how many clicks are needed for a given correction.  The rule of
direction follows from the geometry: a \emph{rear}-sight click moves
the impact in the \emph{same} direction as the click, while a
\emph{front}-sight adjustment (insert height) moves it in the
...
\begin{tikzpicture}[scale=0.75, >=Stealth]
  ...
  \draw[dashed,gray] (11,-2.2) -- (11,2.6);
  \node[above,font=\small] at (11,2.6) {Target ($z_t$)};
  % Original line of sight
  \draw[gray,thin] (2,0) -- (11,0);
  % Rear sight displaced upward by c
  \filldraw[blue] (2,0) circle (2pt)
    node[below left,font=\small] {Rear};
  \filldraw[blue] (2,0.8) circle (2pt)
    node[above left,font=\small] {Rear$'$};
  \draw[<->,thick,blue] (1.5,0) -- (1.5,0.8)
    node[midway,left,font=\scriptsize] {$c$};
  % Sight line after re-centering: passes through rear' and target center
  \draw[red,thick] (2,0.8) -- (11,0);
  \filldraw[red] (5.5,0.489) circle (2pt)
    node[above right,font=\small] {Front$'$};
  % Rotation angle at the displaced aperture
  \draw[->,red,thin] (3.6,0.8) arc[start angle=0,end angle=-6,radius=1.6];
  \node[font=\small,red] at (4.6,0.45) {$\theta = c/L$};
  % Impact shift at the target: bore rotated by theta
  \draw[->,red,very thick] (11,0) -- (11,1.0)
    node[midway,right,font=\scriptsize] {$\Delta = c \cdot R / L$};
  \filldraw[red] (11,1.0) circle (2.5pt)
    node[right,font=\small] {Impact$'$};
  \filldraw[gray] (11,0) circle (2.5pt)
    node[left,font=\small] {aim};
\end{tikzpicture}
\caption{Mechanical sight adjustment geometry.  A rear-aperture click
$c$ tilts the line of sight by $\theta = c/L$ ($L$: sight radius).
Re-centering the sight picture rotates the whole gun---and the bore
with it---by $\theta$, so the impact moves by $\Delta = c\,R/L$ at the
target distance $R$, in the \emph{same} direction as the click.}
\label{fig:clicks}
\end{figure}
...
\]
Because the sight line is re-centered through the displaced aperture,
the impact moves in the \emph{same} direction as a rear-sight click.
A \emph{front}-sight displacement would tilt the line of sight the
other way: re-centering would rotate the gun \emph{against} the
adjustment, and the impact would move \emph{opposite} to it.  (The
exact pivot of the re-centering rotation only contributes
second-order terms; see the wiki page ``La visée au dioptre'', which
develops the same relation from Thales' theorem.)

\subsection{Implementation}
In \file{simulator.js}, the click value (in hundredths of a
millimetre) and the per-click shift are:
\begin{lstlisting}
...
\begin{enumerate}[nosep]
  \item Press the right arrow key several times.  The red dot moves
        right on the target: a rear-sight click moves the impact in
        the \emph{same} direction---$0.5$\,mm per click at 10 m,
        $2.5$\,mm at 50 m with the default value.
  \item Press \key{G}/\key{H} to change the click value and watch the
        per-click shift scale proportionally: the lever-arm ratio
        $R/L$ is fixed by the rifle, the click value by the sight.
